...
if you want all of them to be read then write""", colored('ALL', 'red'),
      """, if not write amount of sentences you would like to be read... """)
...
if len(summary) >= 1:
    if amount_of_sentences_read == 'all' or amount_of_sentences_read == 'ALL' or amount_of_sentences_read == 'All':
        ...
        if amount_of_sentences_read == '1':
            print(colored("here is the full summary --> ", 'yellow'), summary,
                  colored(".... but only the first sentence will be read as you told me to ...", 'yellow'))

        else:
            print(colored('here is the full summary --> ', 'yellow'), summary, colored("... but only first", 'yellow'),
                  colored(amount_of_sentences_read, 'yellow'),
                  colored(' sentences are going to be read, just as you told me to...', 'yellow'))
